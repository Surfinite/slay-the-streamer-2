...
- **Harmony `___field` injection takes the LITERAL field name after three underscores.** StS2 private fields are `_`-prefixed, so `LocTable._name` is `____name` (four underscores). A wrong name makes `PatchAll` throw, and `ModEntry`'s catch turns that into one `FATAL: Init failed` line and a completely inert mod. Both loc patches shipped with `___name` in 2026-09 and never registered; the `Harmony patched N method(s)` block in godot.log is the check.
