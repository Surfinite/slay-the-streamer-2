...
- **Card-reward votes only occur after combat** *(new in v0.2.2)* — when on, chat only votes on card rewards earned from combat. Card-rewards given by events and relics (Orrery, Dream Catcher, the Draft modifier, etc.) are always free streamer picks — no vote, no skip budget. On by default; turn it off to restore voting on every card reward.
