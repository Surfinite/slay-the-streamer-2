...
- `Ti/Voting/VoteCoordinator.cs` — instance-based session owner. Constructor takes `(IChatService chat, IMainThreadDispatcher dispatcher, ITimerScheduler scheduler, IClock clock, Func<VoteSnapshot, ReceiptKind, string>? formatReceipt = null, Random? random = null)`. `Start(label, options, duration, receipts?, parsing?, ct?)` opens a session; throws if one is already open on the coordinator.
