# YouTube chat parallel integration (design v4)

**Date**: 2026-05-12
**Status**: Draft v4 — post-round-2 5-reviewer meta-review on v3. 11 Must-do + 17 Should-do items applied. Three Consider items at end as pick list. See [`META-REVIEW-round2-2026-05-12-youtube-chat-integration-design.md`](./META-REVIEW-round2-2026-05-12-youtube-chat-integration-design.md) for the round-2 consensus/contradiction analysis. Implementation-ready pending user disposition on the three Consider items.
**Predecessor**: [`2026-05-12-youtube-chat-integration-design-v3.md`](./2026-05-12-youtube-chat-integration-design-v3.md).
**Scope**: Unchanged from v1–v3.

## Author's note on v4 changes

Round-2 meta-review (5 reviewers) found two real-correctness gaps the v3 spec missed:

1. **`VoteSession.cs` line 85 subscribes to `IChatConsumer.ConnectionStateChanged`** for disconnect-gap tracking. v3's no-op explicit-impl on `MultiChatService` would have **silently broken** disconnect-gap tracking when chat is wrapped in a multi. v4 restores proper aggregate event emission in `MultiChatService` with `_lastAggregateState` caching. (Round-2 C-2; verified against `src/Ti/Voting/VoteSession.cs`.)
2. **`VoteTallyLabel` filtered platforms by observed tally keys**, contradicting Decision 10's "configured-not-observed" gating. A configured-but-silent YT row wouldn't render until first YT vote — the same mid-vote rendering snap v2/v3 were supposed to fix, but for a different reason. v4 uses `ConfiguredPlatforms` list (replacing `IsMultiPlatformConfigured` bool per Round-2 C-7) and iterates over it explicitly.

Plus a stack of contract-tightening fixes that emerged from round-2:
- Escalation receipt mechanism pinned to **event-only** (drop static-facade alternative); TI/Game seam preserved.
- Vote ID visible in `VoteTallyLabel` header (`Chat voting [07] — 30s left`). v3's vote-nonce was invisible to YT viewers (the people it most helps) because YT is read-only.
- `YouTubeChatStatusReason` mechanically wired into `TransitionTo` signature; all call sites pass an explicit reason.
- Typed `YouTubeHttpStatusException` for 429 handling (replaces `HttpRequestException` shim — was mechanically brittle).
- Latest-wins per-platform tally update algorithm spelled out explicitly.
- `InvalidOrUnavailableChannel` enum value removed (dead code; conflicts with D7).
- Malformed `youtubeChannelId` now disables YT only (not whole mod) per D6 v4 — matches the "all YT failures → Twitch-only" philosophy.
- C4 telemetry simplified to single-location tracker (was over-engineered with dictionary).
- Cross-thread/concurrency subsection added for `YouTubeChatService` per project Interlocked convention.

Round-2 also caught the C3 scope-honesty issue (R2): vote-nonce is a Plan A vote-engine change touching ~7 files, not "~40 LOC in `Ti/Voting/`." Decision 11 now acknowledges this.

Three Round-2 Consider items at the end: 3-digit VoteId, vote-nonce explainer receipt, `LastError` timestamp tracking. All lean-no or lean-neutral.

## Author's note on v3 changes (preserved for review-trail continuity)
...
| 6 | **Settings JSON: `youtubeChannelId` (optional, nullable string); malformed disables YT only.** <!-- CHANGED v4 per Round-2 O-1 / Conflict 5: malformed YT field no longer degrades the whole mod --> | Missing field OR JSON `null` → YT disabled (`Success`, NOT `Malformed`). Non-empty trimmed → preserved. **Trim first**: empty-after-trim → clamp to null (`Success`, no warning). Post-trim non-empty containing control characters → log Warn ("youtubeChannelId malformed; YouTube integration disabled, Twitch continues") + clamp to null + add a warning to `SettingsResult.Success.Warnings`. **YT is disabled, but Twitch (and the rest of the mod) continues normally.** Rationale: `youtubeChannelId` is an optional, non-secret field; the philosophy of "all YT failures degrade to Twitch-only" applies to malformed settings too. Hostage-taking the whole mod for a single bad optional field would be hostile UX. Escape-hatch `youtubeVideoIdOverride` still rejected. |
...
| 11 | **Vote-nonce / per-vote ID — `Vote [07]: #0 Strike, #1 Defend...` receipt format; opt-in `!NN` parsing suffix.** <!-- NEW v3 / refined v4: zero-padding clarified per Round-2 C-14 --> | Each `VoteSession` gets a `VoteId` (int, 0–99 cycling per `VoteCoordinator.Start` call). Display format is **zero-padded 2-digit** (`[07]`, `[42]`, `[99]`). Vote-parsing regex accepts optional `!NN` suffix: `#0` or `0` (bare) → current vote; `#0!42`, `0!42`, `#0!4` (also accepted, padding optional in parsing), `0!04` → match vote with that ID; dropped with Debug log if `VoteId != nonce` OR if nonce is outside 0–99 (defensive). **Backward-compatible**: bare `#N` continues to work, preserving the StS1 "Skip Gang" `#0 = skip` convention. **Supersedes** the Noita-pattern alternating-numbers v0.2 entry in `notes/06-followups-and-deferred.md`. Close-receipts unchanged (`Chat chose Strike`; vote-ID omitted from close). **Scope acknowledgment** (per Round-2 C-17): vote-nonce is a Plan A vote-engine change touching ~7 files (`VoteSession`, `VoteCoordinator`, `EnglishReceipts`, `VoteSnapshot`, regex/`VoteParsingPolicy`, tests, `VoteTallyLabel` header), not just "~40 LOC in `Ti/Voting/`." Estimated ~60-80 LOC source + ~30 LOC tests across `Ti/Voting/` + `Ti/Ui/`. It's included in this slice because YT latency makes the pre-existing Noita problem acute. |
...
public enum YouTubeChatStatusReason {
    ...
    // <!-- REMOVED v4 per Round-2 C-9: InvalidOrUnavailableChannel was never emitted (collapsed to
    //      NoLiveBroadcastFound) and conflicted with D7's anti-disambiguation posture. Add back in
    //      v0.2 if/when permanent-404 disambiguation is implemented. -->
}
```

`YouTubeChatService.LastStatusReason { get; private set; }` is updated via `TransitionTo(state, reason, statusReason)`. **Every call site MUST pass an explicit `YouTubeChatStatusReason`** (no default-None for transitions that have a known cause). Reset to `None` on successful `ConnectedReadOnly` transition. <!-- CHANGED v4: per Round-2 C-5; v3 had the property but didn't mechanically force callers to pass it -->

```csharp
private void TransitionTo(
    ChatConnectionState next,
    string reason,
    YouTubeChatStatusReason statusReason)   // <!-- CHANGED v4: required, not optional -->
{
...
All transition sites pass an explicit reason:
- Discovery null → `NoLiveBroadcastFound`
- Poll returns no continuation → `LiveBroadcastEnded`
- `HttpRequestException` / `TaskCanceledException` (timeout) → `NetworkError`
- `YouTubeHttpStatusException(429)` → `RateLimited`
- Scraper parse failure (regex no-match, JSON path missing) → `ScraperParseFailed`
- Generic catch → `UnknownError`
- Successful `ConnectedReadOnly` → `None` (resets after recovery)

Tests assert `LastStatusReason` for each failure path.
...
Receipt text (per Round-2 O-5 — generic wording, no specific "30 min" claim which is wrong under 429 backoff): <!-- CHANGED v4 -->
```
YouTube: still not connected after repeated retries — check "youtubeChannelId" in settings (see logs for details).
```

**Mechanism — pinned as event-only per Round-2 C-1**: <!-- CHANGED v4: drop the static-facade alternative; static facade would create a Ti/Chat → ModEntry reverse dependency, violating the TI/Game seam this spec exists to preserve. -->
...
`ModEntry` subscribes to `EscalationRequested` and routes the receipt text to the Twitch child's `SendMessageAsync` at priority High. Tests pass a mock subscriber. **No static facade**, no `ModEntry.SendEscalationReceipt()` callback.
...
### Typed HTTP status exception <!-- NEW v4 section per Round-2 C-6 -->

`IYouTubeHttp` throws a typed exception on non-success responses, preserving `StatusCode` and `Retry-After`:
...
internal sealed class YouTubeHttpStatusException : Exception {
    ...
    public YouTubeHttpStatusException(HttpStatusCode statusCode, TimeSpan? retryAfter, string message)
        : base(message) { StatusCode = statusCode; RetryAfter = retryAfter; }
}
```

`IYouTubeHttp.GetWithRedirectAsync` and `PostJsonAsync`:
```csharp
var response = await _client.SendAsync(request, ct);
if (!response.IsSuccessStatusCode) {
    var retryAfter = response.Headers.RetryAfter?.Delta;   // or compute from RetryAfter.Date
    throw new YouTubeHttpStatusException(response.StatusCode, retryAfter,
        $"HTTP {(int)response.StatusCode} from {request.RequestUri}");
}
return response;
```

This makes 429 detection mechanically correct (`is YouTubeHttpStatusException { StatusCode: HttpStatusCode.TooManyRequests }`) and preserves the `Retry-After` header value for the cadence computation. v3's `HttpRequestException` inspection (which doesn't reliably carry status code or headers) is replaced.

### Reconnect cadence (per D7 + 429 carve-out)
...
private TimeSpan NextReconnectDelay(Exception? lastError) {
    // <!-- CHANGED v4: keys on YouTubeHttpStatusException per Round-2 C-6 -->
    if (lastError is YouTubeHttpStatusException { StatusCode: HttpStatusCode.TooManyRequests } ex) {
        _consecutive429Count++;
        if (ex.RetryAfter is { } retryAfter && retryAfter > TimeSpan.Zero) {
            // Honor server's Retry-After. Apply positive-only jitter (don't undershoot the server's request).
    ...
    // <!-- CHANGED v4: per Round-2 C-16; reset on any non-429 outcome, not just 2xx -->
    _consecutive429Count = 0;
    return ClampJitter(ReconnectBase);
}
```

`_consecutive429Count` resets on any non-429 outcome (a 500, a network error, a successful 2xx all reset). The escalation receipt counter `_consecutiveReconnectCount` resets only on a successful `ConnectedReadOnly` transition.

### Poll cadence (steady-state)

Unchanged from v1 (floor 1s, ceiling 10s, honor `timeoutMs`).

### Initial-poll backlog suppression — explicit flow <!-- CHANGED v4 section per Round-2 C-19 -->

To resolve the "which poll is first?" ambiguity from v3:

**Pinned flow** (the cursor-establishing poll happens BEFORE the `ConnectedReadOnly` transition; the steady-state loop emits from the second poll onward):

1. `ConnectAsync(channelId)` → `Connecting`.
2. `_discovery.FindLiveVideoIdAsync(channelId)` returns `videoId`.
3. `_scraper.ParseInitialPageAsync(videoId)` returns `(apiKey, clientVersion, initialContinuation)`.
4. `_scraper.PollAsync(apiKey, clientVersion, initialContinuation)` returns `(messages, nextContinuation, nextTimeoutMs)`. **This is the cursor-establishing poll.** Its `messages` collection is discarded with a Debug log. `nextContinuation` is saved as `_continuation`.
5. Transition `Connecting → ConnectedReadOnly` with reason `None`.
6. Steady-state loop starts; first iteration polls with `_continuation` from step 4; emits any messages from this poll onward.

This way `_firstPollAfterConnect` is unnecessary — the cursor-establishing poll is structurally part of the connect flow, not a flag-gated first iteration of the steady-state loop.

**Reconnect after `Reconnecting`**: same flow. Step 2 re-runs discovery (`_videoId` cleared on reconnect per D7); steps 3–6 follow.

**Trade-off (per Round-2 C-23)**: messages that arrived in the window between page-load (step 3) and the cursor-establishing poll (step 4) — typically <1s — are dropped. Estimate ≤1-3% of YT votes during the brief connect window; acceptable cost to prevent backlog pollution. Operator-validation should measure this.

Add tests:
- `InitialPollMessagesAreNotEmitted` — assert messages from step 4 do not raise `MessageReceived`.
- `SteadyStatePollEmitsMessages` — assert subsequent polls do raise `MessageReceived`.
- `ReconnectAfterBroadcastEnd_RepeatsTheFlow` — assert the discard happens on the reconnect cursor-establishing poll too.

### Concurrency / lifecycle <!-- NEW v4 section per Round-2 C-12 -->

Per the codebase convention (CONTEXT §5: "Interlocked-int flags for cross-thread state, not bool"), `YouTubeChatService` uses:

```csharp
private int _disposed;                          // 0 = live, 1 = disposed; Interlocked.Exchange/Read
private ChatConnectionState _state;             // mutated only through TransitionTo on dispatcher
private CancellationTokenSource? _cts;          // protected by lock for swap-during-reconnect
private readonly object _lifecycleLock = new();
```

**State change discipline**: `_state` is only mutated inside `TransitionTo`. `TransitionTo` posts the `ConnectionStateChanged` event via `_dispatcher.Post` so subscribers always receive it on the main thread (matches Twitch's pattern).

**Dispose race**: `Dispose()` performs `Interlocked.Exchange(ref _disposed, 1)`. The retry timer callback, poll-loop continuation, and `ConnectAsync` re-entry all check `Volatile.Read(ref _disposed) == 1` at entry and no-op if disposed.

**Reconnect generation**: `_cts` is swapped on reconnect (`_cts?.Cancel(); _cts = new();` under `_lifecycleLock`). The poll-loop captures `var localCts = _cts;` once per iteration so a stale poll continuation can't publish messages after a reconnect cycle started.

**Timer cleanup**: `_retryTimer?.Dispose()` is called at the start of every `ArmReconnect()` (preventing duplicate timers) and in `Dispose()`.

Lifecycle table (per Round-2 R4 additions):

| Event | Required behavior |
|---|---|
| `ConnectAsync` while `Disconnected` | starts discovery |
| `ConnectAsync` while `Connecting` or `ConnectedReadOnly` | no-op (idempotent) |
| `Disconnect()` while `Connecting` | cancels discovery, transitions to `Disconnected` |
| `Disconnect()` while poll-loop active | cancels CTS, poll loop exits cleanly, transitions to `Disconnected` |
| `Dispose()` while retry timer armed | disposes timer, sets `_disposed`, no further transitions |
| Retry timer fires after `Dispose()` | check `_disposed`, no-op |
| Poll response arrives after CTS canceled | check generation/CTS, discard |
| `EscalationRequested` subscribers throw | caught + logged Error; doesn't affect state machine |

**ConnectAsync safety contract** (per Round-2 C-13): `ConnectAsync` MUST NOT throw for external failures (network errors, discovery failures, etc.). Synchronous validation errors (e.g., `channelId` null) may throw `ArgumentException` synchronously. All other failures route through `TransitionTo(Reconnecting, ...)` and `ArmReconnect()`. `ModEntry`'s `_ = youtube.ConnectAsync(...)` fire-and-forget is therefore safe.
...
5. **Health-check telemetry**: <!-- CHANGED v4 per Round-2 C-10 (simplify) + C-20 (PII redaction) --> single failure-location tracker `(string? lastFailureLocation, int consecutiveCount)`. On parse failure, if the location matches `lastFailureLocation`, increment count; if different, reset to (newLocation, 1). On successful parse at any location, reset to (null, 0). When count reaches N=5, log Error one-shot with a **structural sample** (NOT raw response content):
   ```
   [YouTubeLiveChatScraper] 5 consecutive parse failures at INNERTUBE_API_KEY_regex; structural sample: response JSON keys at top level = [responseContext, contents, trackingParams], renderer types observed = [liveChatTextMessageRenderer]
   ```
   The structural sample logs **JSON top-level keys, renderer type names observed, response length** — never raw message text, author names, or channel IDs. Speeds post-redesign maintainer diagnosis without leaking PII. ~15 LOC; uses existing `TiLog.Error`. (v3's `Dictionary<string,int>` per-location counter was simplified to a single tuple; <5 distinct failure points means a dict is over-allocation.)
...
public sealed class MultiChatService : IChatConsumer {  // <!-- CHANGED: IChatConsumer not IChatService -->
    ...
    public IReadOnlyList<string> ConfiguredPlatforms { get; }   // <!-- NEW v4: per Round-2 C-7 -->

    public MultiChatService(params (string Name, IChatConsumer Service)[] children) {
        // <!-- CHANGED v4: per Round-2 C-21; explicit validation with clear messages -->
        ...
        // <!-- CHANGED v4: per Round-2 C-15; store handler references so Dispose can unsubscribe -->
    ...
    // <!-- CHANGED v4: per Round-2 C-2 / Conflict 1; v3's no-op explicit-impl was a contract lie that would
    //      have broken VoteSession.cs's disconnect-gap tracking (which subscribes to ConnectionStateChanged).
    //      v4 emits the aggregate event properly with _lastAggregateState caching. -->
    ...
    private void OnChildConnectionStateChangedInternal(string name, object? sender, ChatConnectionChangedEventArgs e) {
        // Compute aggregate after the child's mutation has already landed.
        var newAggregate = AggregateState();
        var oldAggregate = _lastAggregateState;
        _lastAggregateState = newAggregate;

        // Always fire per-child event for ModEntry's D8 receipt routing.
        ...
        // Fire aggregate event only when aggregate state actually changed.
        if (newAggregate != oldAggregate) {
            var aggArgs = new ChatConnectionChangedEventArgs(oldAggregate, newAggregate, $"child '{name}' transition triggered aggregate change");
    ...
    // <!-- CHANGED v4: per Round-2 C-22; log Warn on unknown name so typos don't silently disable the gate -->
    ...
    public async Task SendMessageAsync(string text,
        OutgoingMessagePriority priority = OutgoingMessagePriority.Normal,
        CancellationToken ct = default) {
        // <!-- CHANGED v4: per Round-2 C-18; differentiate "zero CanSend" (expected in YT-only mode)
        //      from "all attempted failed" (receipt dropped). Switched from ContinueWith to async/await
        //      for clearer exception/cancellation handling per Round-2 minor. -->
        var sendable = _children.Values.Where(c => c.CanSend).ToList();
        if (sendable.Count == 0) {
            TiLog.Debug("[MultiChatService] SendMessageAsync: no CanSend children; receipt skipped (expected in YT-only mode)");
            return;
        }
        int successCount = 0;
    ...
    public void Dispose() {
        // <!-- CHANGED v4: per Round-2 C-15; unsubscribe before disposing children -->
...
| 5 | `AuthenticationFailed` | All children terminal, at least one in `AuthenticationFailed` |
| 6 | `JoinFailed` | All children terminal, at least one in `JoinFailed` |
| 7 | `Disposed` | All children in `Disposed` |
| 8 (worst) | `Disconnected` | None of the above matches |

**Mixed-terminal fall-through**: When all children are in terminal states but the states differ, use the priority ranking `AuthenticationFailed > JoinFailed > Disposed > Disconnected` and return the highest-priority terminal state observed in any child. <!-- CHANGED v4: per Round-2 Conflict 3; AuthFailed is more user-actionable than Disposed, so it ranks higher even though both are "all-children" rules. Since skip-gate now routes through Twitch-specific GetChildState (not aggregate), this ranking is diagnostic-only — but more actionable ordering is still preferred. --> Mixed-terminal observation also logs a Warn so the operator notices the split.
...
// Nonce check: if present, must match this vote's ID; out-of-range nonce dropped.
// <!-- CHANGED v4 per Round-2 C-14: explicit range-check, accept both !4 and !04 -->
...
`EnglishReceipts.FormatOpen`: <!-- CHANGED v4: zero-padded VoteId per Round-2 C-14 -->
```csharp
// "Vote [07]: #0 Strike, #1 Defend, #2 Bash — 30s, type #N or N"
return $"Vote [{snapshot.VoteId:D2}]: {options}— {duration:%s}s, type #N or N";
```
...
## `VoteSession` per-platform tally (with `ConfiguredPlatforms`) <!-- CHANGED v4: per Round-2 C-7; replaces v3's IsMultiPlatformConfigured bool -->
...
public sealed class VoteSession : IDisposable {
    ...
    private readonly IReadOnlyList<string> _configuredPlatforms;   // <!-- CHANGED v4: list, not bool -->
    private readonly Dictionary<string, DateTimeOffset> _lastVoteByPlatform = new();
    public int VoteId { get; }                  // <!-- NEW v3: Decision 11 -->
    public int TallyVersion { get; private set; }   // <!-- NEW v4: per Round-2 #16; for VoteTallyLabel cache invalidation -->

    internal VoteSession(..., IReadOnlyList<string> configuredPlatforms, int voteId) {
        ...
        // This is what gives the UI configured-but-silent rows to render (Round-2 C-3 fix).
...
`VoteCoordinator` takes `IReadOnlyList<string> configuredPlatforms` as an explicit constructor parameter (no type-inspection of `IChatConsumer` to derive it). `ModEntry` passes it: <!-- CHANGED v4: per Round-2 R5 — resolves v3's derivation contradiction -->
...
var voter = new VoteCoordinator(chat, configuredPlatforms, ...);
```

### Latest-wins per-platform tally update algorithm <!-- NEW v4 section per Round-2 C-11 -->

In `VoteSession.OnChatMessage`, after the merged `_tallies[idx]++` and before the `TallyChanged` event raise:

```csharp
var platform = PlatformOf(msg);     // "twitch" or "yt:"-prefix → "youtube"

// Latest-wins decrement of prior per-platform vote.
// Platform is voter-stable (same VoterKey → same prefix → same platform),
// so we don't need to store platform per voter — current msg's platform == prior msg's platform.
if (existing) {
...
**Invariant**: `sum(_talliesByPlatform.Values) == sum(_tallies.Values)` after every accepted vote. Asserted in tests.
...
public override void _Process(double delta) {
    ...
    // <!-- CHANGED v4: per Round-2 C-4; vote-ID in header so YT viewers (who don't see Twitch receipts) can use !NN syntax -->
    ...
    if (perPlatform is null) {
        ...
        // Multi-platform — iterate ConfiguredPlatforms in registration order.
        // <!-- CHANGED v4: per Round-2 C-3; iterate over configured (not observed) so silent
        //      platforms still render with zero rows. PlatformDisplayOrder is gone; use the
        //      coordinator-supplied registration order instead. -->
...
<!-- CHANGED v4: PlatformDisplayOrder constant removed — iteration order is now ConfiguredPlatforms (registration order from ModEntry). This eliminates the v3 magic-string ordering nit and is more correct for future-platform additions. -->
...
- Post-trim non-empty containing any control character (`char.IsControl(c)` true) → **log Warn + clamp to null + add a warning to `SettingsResult.Success.Warnings`**. Whole mod stays on `Success`; YT is disabled, Twitch continues. <!-- CHANGED v4 per Round-2 O-1 -->
...
- Whitespace-only (`"   "`) → trimmed to empty → clamped to null, `Success`, no warning.
- Whitespace surrounding valid ID (`"  UCabc123  "`) → trimmed to valid value, preserved.
- Embedded control char in trimmed value → `Success` with warning + YT disabled (Twitch continues). <!-- CHANGED v4 per O-1 -->
...
| 1c | `youtubeChannelId` non-empty trimmed but contains control chars | <!-- CHANGED v4 per Round-2 O-1 --> `Success` with warning logged; YT disabled (clamped to null); Twitch continues normally. No longer degrades whole mod. |
| 1d | **Initial-poll backlog suppression drops new messages** | <!-- NEW v4 per Round-2 C-23 --> Messages arriving between page-load and the cursor-establishing poll (~<1s window) are silently dropped. Acceptable trade-off vs. backlog pollution. Estimated ≤1-3% of YT votes per connect cycle. Operator-validation measures the actual rate. |
...
**v4 resolution**: <!-- CHANGED v4 per Round-2 C-8: v3's stale "Consider C3 lean-no" wording removed; v3 promoted C3 to Decision 11, this v4 just clarifies the operator-validation expectation. -->

- **Vote-nonce (Decision 11) is the chosen mitigation.** Backward-compatible `!NN` suffix; bare `#N` preserves Skip Gang.
- **Vote-echo on tally label** (already applied) supplements: gives streamer visual heartbeat that YT votes are landing.
- **`notes/06` Noita-pattern alternating-numbers entry is superseded by Decision 11.** Update notes/06 post-implementation to mark it superseded with a pointer to this spec.
- **Operator-validation should still measure** whether bare-`#N` (no nonce) usage by YT viewers causes visible wrong-vote issues despite nonce availability. If it does, the streamer can verbally encourage `!NN` syntax. If it remains widespread, v0.2 could add a stronger default (e.g., mandatory nonce when both platforms configured).
...
- [`docs/superpowers/specs/META-REVIEW-2026-05-12-youtube-chat-integration-design.md`](./META-REVIEW-2026-05-12-youtube-chat-integration-design.md) — round-1 v1 → v2 reasoning.
- [`docs/superpowers/specs/META-REVIEW-round2-2026-05-12-youtube-chat-integration-design.md`](./META-REVIEW-round2-2026-05-12-youtube-chat-integration-design.md) — **round-2 v3 → v4 reasoning** (consensus / contradictions / Consider pick list).
...
- [`docs/superpowers/specs/2026-05-12-youtube-chat-integration-design-v2.md`](./2026-05-12-youtube-chat-integration-design-v2.md) — v2 (post-round-1 meta-review).
- [`docs/superpowers/specs/2026-05-12-youtube-chat-integration-design-v3.md`](./2026-05-12-youtube-chat-integration-design-v3.md) — v3 (post-Consider-tier user selections; predecessor to v4).
...
## v4 Optional Enhancements (pick list) <!-- NEW v4 -->

Three Consider-tier items from the round-2 meta-review. Reply with "also apply CC1, CC3" (etc.) to fold any in; or leave dropped.

### CC1. 3-digit VoteId
- **What**: `[042]` zero-padded 3-digit instead of `[07]` 2-digit. Cycle window becomes ~8.3 hours at 30s/vote instead of 50 minutes.
- **Reviewers**: R4 (alt C). 1/5.
- **Effort**: Trivial (~3 LOC; change `D2` → `D3` in formatters; expand parser range to 0-999).
- **Recommendation**: **Lean no.** 50 minutes of disjoint nonce-space is more than enough — YT chat delay is seconds, not minutes. Stay 2-digit.

### CC2. Vote-nonce explainer one-time receipt
- **What**: At first vote of session (or once per process), fire a single Twitch receipt: `Tip: append "!07" to vote in vote 07 specifically (helps stream-delayed viewers).` Priority Low.
- **Reviewers**: R5 (post-C5-drop sanity check). 1/5.
- **Effort**: Trivial (~10 LOC).
- **Recommendation**: **Lean neutral.** Adds Twitch chat noise (which is why round-1 C5 was dropped); but unlike C5 this only fires once and addresses a known confusion case. User's call — same trade-off as C5 round-1.

### CC3. `MultiChatService.LastError` timestamp-based most-recent
- **What**: Replace `LastError => null` with tracking the most-recent child error via timestamps. Returns the freshest error across children.
- **Reviewers**: R4 (Should-do #14). 1/5.
- **Effort**: Small (~20 LOC; subscribe to children, cache `(Error, Timestamp)` on each non-null update).
- **Recommendation**: **Lean no.** v3's null + GetChild query is honest about the aggregation problem. Adding timestamp state for diagnostic-only data isn't worth it for v1. Revisit in v0.2 if diagnostics prove painful.

---

## v3 Optional Enhancements — disposition (preserved for review-trail continuity)
...
### Detail entries (v2 pick list — **obsolete, kept for historical context only**)

> **NOTE v4**: The recommendation columns ("Lean yes", "Lean no", etc.) below reflect v2's analysis when items were Consider-tier. In v3, the items marked "APPLIED" in the table above became part of the spec proper. **Do not re-evaluate these recommendations** — they are frozen v2 state. The current dispositions are in the v3 disposition table above (and the v4 disposition is in the v4 Optional Enhancements section at the top).
...
**v4 status**: 11 Must-do + 17 Should-do items from the round-2 meta-review applied. Three Consider items (CC1-CC3) presented as pick list above. Spec is implementation-ready pending user disposition on those three; round-2 reviewers R3 and R5 explicitly endorsed v3 as implementation-ready, and v4 layers fixes on top of that endorsement.
