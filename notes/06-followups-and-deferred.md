...
- [ ] **Skip receipt wording**: chat currently sees `Streamer skipped a card reward (1/1 act)` at the moment `AfterOverlayClosed` fires — which (per spike) is the next-room-transition, not the Proceed click. By that point the on-screen counter label has been QueueFreed (in `AfterOverlayClosed` postfix) and the next rewards screen hasn't opened yet, so chat reads the `(1/1 act)` annotation with no visible reference. Surfinite flagged 2026-05-11 that this is "probably fine, just reword a little". Candidate rewordings: `Streamer skipped a card reward — 0 skips remaining this act` or `Streamer skipped a card reward (used 1 of 1 this act)` — both stand alone without depending on the on-screen counter being visible.
