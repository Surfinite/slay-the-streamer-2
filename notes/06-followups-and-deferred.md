...
## v0.111.0 compat pass: ZERO mod impact (verified 2026-08-14)

Game Beta v0.110.0 → v0.111.0: 172 changed / 23 added / 14 removed files (netcode handshake rework, VFX/orb namespace moves, balance). ~20 files intersect the bound surface; every bound member verified intact, including the v110 cross-branch lesson's shape check: `StartRunLobby.Players` still `List<StartRunLobbyPlayer>`, `Act1` untouched, `NCharacterSelectScreen`/`NCustomRunScreen.OnEmbarkPressed` untouched, `MegaSprite` unchanged, `Hook.AfterRewardTaken` still has zero awaiting overrides (Bossy Relics assumption holds). Surfinite validated the live Workshop build on v0.111.0 same day — release v0.2.1's tested-against pin updated in place via `gh release edit` (v0.1.2 precedent, no re-release). Notable non-breaking: `TreasureRoomRelicSynchronizer` pick-while-inactive softened throw→warn (mildly helpful to chest patch); `PreloadManager.LoadRoomCombatAssets` now throws pre-monster-generation (we only use `Cache.GetScene`). False alarm worth remembering: the `PlayerAgreedToModLoading` / `NConfirmModLoadingPopup` consent gate is NOT new in v0.111 — it predates v0.110 (serialized as `mods_enabled`; long-since accepted on this machine). v0.111 only added a main-menu popup trigger for profiles with no `ModSettings` block at all. **Lesson: before flagging a diff hunk as "new behavior", grep the OLD decompile for the referenced members — a changed line is not a new mechanism.**

---

## v0.110.0 cross-branch compat: `StartRunLobby.Players` (v110-compat, 2026-08-01)

Game v0.110.0's lobby refactor changed `StartRunLobby.Players` from `List<LobbyPlayer>` to `List<StartRunLobbyPlayer>`. A mod built against v0.110 throws `MissingMethodException` calling it on the default branch (v0.107.1) — thrown at JIT time from the CALL SITE in `ActVariantVotePatch.Prefix` (outside `TryGetPlayerCount`'s own try/catch), so the prefix's catch passed through to vanilla and the act-variant vote silently never fired on the default branch. Curses/card/boss/ancient votes were unaffected (`RunState.Players` uses the unchanged `Player` type). Surfaced by Surfinite testing Cursed Overrides on the default branch 2026-08-01; fixed `v110-compat/1` by binding `Players` reflectively and counting via non-generic `ICollection` (SeedCompat pattern). **Game-update compat passes must check the member diff against BOTH live branches, not just old-vs-new** — any changed generic return type on a directly-called member is a cross-branch break even when the call site "still compiles."
...
- [x] ~~**`TiLog.Sink` should scrub `ex.ToString()`** before forwarding.~~ **Resolved 2026-05-25.** `TiLog.Scrub` promoted from `private` to `public` ([`src/Ti/Internal/TiLog.cs:27`](../src/Ti/Internal/TiLog.cs#L27)); ModEntry's Error branch now wraps `ex.ToString()` in `TiLog.Scrub(...)` before interpolating ([`src/ModEntry.cs:82`](../src/ModEntry.cs#L82)). Note: `OauthPattern` still only matches the `oauth:<token>` shape — bare bearer tokens or YouTube cookies would still leak if they ever appeared in exception text. Broadening the regex deferred (false-positive risk on base64-ish strings).
