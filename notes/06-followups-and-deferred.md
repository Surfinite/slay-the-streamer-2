...
- **YouTube chat parallel integration.** Per ArmadilloTea: FrostPrime wants YouTube chat participation alongside Twitch. YouTube's API is more restrictive than IRC, so research item: (a) what YouTube live-chat read API options exist (likely Polling at low cadence via Data API v3, or community-membership-only via newer streams); (b) reuse `VoteCoordinator` with a `YouTubeChatService` that implements `IChatService`; (c) consider merging tallies across both chats or running them as separate parallel votes. Should reach out to FrostPrime when his tournament is over (per ArmadilloTea) to confirm priority and offer to help if he ends up commissioning a different mod author.
