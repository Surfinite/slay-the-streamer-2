...
- [ ] **`cardSkipsPerAct: 0` strict mode has a sub-screen Skip back-door.** The parent rewards screen's Skip is correctly disabled by `DisallowSkipping()`, but the card sub-screen (`NCardRewardSelectionScreen`) has its own Skip button (an "alternate reward" via `OnAlternateRewardSelected(PostAlternateCardRewardAction.DismissScreenAndRemoveReward)`). Decision 12 in the spec deliberately leaves alternate buttons unpatched (to allow reroll as a streamer escape valve), so the sub-screen Skip remains clickable even in strict mode. **Fix for v0.2**: patch `OnAlternateRewardSelected` prefix that returns false (skip vanilla) when `ShouldEnforceSkipGate() && !_tracker.IsSkipAllowed(settings.CardSkipsPerAct)`. This would close the back-door while leaving alternate paths working in non-strict mode. Requires Decision 12 amendment in the v2.1 spec.
