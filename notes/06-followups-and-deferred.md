...
- [ ] **Twitch message-burst pacing on cancellation paths**. Confirmed 2026-05-11 via operator-validation Step 6 sub-step B (run-abandon mid-vote): the close-receipt + cancellation-receipt pair fires within ~1s of each other, and if a periodic-tally receipt fired in the last ~7s of the vote, that's 3 messages in ~10s. Default Twitch ratelimit (20/30s) tolerates this, but **streamer channels with slowmode enabled will drop at least one of the three** with a `Twitch ratelimit/slowmode NOTICE: Your message was not sent because you are sending messages too quickly` (silent on the user-facing chat side; only visible in `godot.log`). Observed dropped messages have been unpredictable — sometimes the periodic tally, sometimes the close receipt, sometimes the cancellation itself. Polish for v0.2: (a) suppress periodic tally if `TimeRemaining < closeReceipt + cancellation slack` (~5s); (b) consider merging close + cancellation into a single composite receipt when the cancellation path is known at close time (would need VoteSession-level awareness — bigger change); (c) at minimum, raise the cancellation-receipt priority from Normal to High so the queue de-prioritizes the periodic tally instead.
