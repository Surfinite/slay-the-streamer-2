...
- [ ] **`cardSkipsPerAct: 0` strict mode has a sub-screen Skip back-door** AND **mid-vote sub-screen Skip allows abort-and-retry exploit**. Two related issues with the same root cause and fix.
  - Strict-mode back-door: parent rewards screen's Skip is correctly disabled by `DisallowSkipping()`, but the card sub-screen (`NCardRewardSelectionScreen`) has its own Skip button (alternate reward via `OnAlternateRewardSelected(DismissScreenAndRemoveReward)`). Decision 12 leaves alternate buttons unpatched (to allow reroll as a streamer escape valve), so the sub-screen Skip remains clickable even in strict mode.
  - Mid-vote abort-retry exploit: if a card vote is in progress and the streamer clicks the sub-screen Skip, vanilla skips the card → our `RewardSkippedFrom` postfix would have decremented the budget. **Partial mitigation in commit (this work)**: skip the budget decrement when `CardRewardVotePatch.VoteInProgress` (mid-vote skip stays free in the budget sense; the vote still drops via the resume's IsInstanceValid check). But the streamer can keep aborting+retrying as long as the card button is re-clickable on the parent screen.
  - **Full fix for v0.2**: patch `OnAlternateRewardSelected` prefix that returns false (block vanilla) when `ShouldEnforceSkipGate() && (CardRewardVotePatch.VoteInProgress || !_tracker.IsSkipAllowed(settings.CardSkipsPerAct))`. This closes both the strict-mode back-door AND the abort-retry loop. Requires Decision 12 amendment in the v2.1 spec.
