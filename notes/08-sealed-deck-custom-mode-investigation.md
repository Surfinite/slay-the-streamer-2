...
- **Viability verdict for v1: use vanilla as-is is realistic for most characters, but Necrobinder is a structural exception.** Streamer selects Custom Mode → ticks `SealedDeck` → embarks → drafts 10 of 30 → run continues, our existing B.2.1 patches still apply. **However**: `Bodyguard` and `Unleash` are `CardRarity.Basic`, and Basic-rarity cards never roll in `RegularEncounter` odds — so Necrobinder's identity cards are **structurally absent** from any vanilla SealedDeck draft, making the character unplayable in this mode without mod-side intervention. Other characters likely have the same problem in less severe form (any character whose StartingDeck contains non-Strike/Defend Basics). See the "Caveats" subsection under Viability assessment for the structural diagnosis and the Q7 open-question resolution.
...
**Verdict: PARTIAL — viable for most characters, but Necrobinder needs mod-side intervention to be playable at all.** The procedural shape of the run works on every character (Custom Mode → SealedDeck modifier → 10-of-30 draft → run continues). For Necrobinder specifically, the draft pool structurally cannot contain `Bodyguard` or `Unleash` (both `CardRarity.Basic`, excluded from `RegularEncounter` odds), so the character has no Osty mechanic. Other characters with non-Strike/Defend Basics in their StartingDeck likely have similar but less-severe gaps. **If v1 ships without per-character must-include support, document Necrobinder as "not playable in sealed-deck mode under this version"; do not silently let streamers pick it and get stuck.** Surfinite's earlier description of must-includes as "configurable" is the right shape; it just moves from optional to load-bearing.
...
2. **Necrobinder is structurally unplayable under vanilla SealedDeck.** Confirmed 2026-05-12: `Bodyguard` ([Bodyguard.cs:18](../decompiled/sts2/MegaCrit/sts2/Core/Models/Cards/Bodyguard.cs#L18)) and `Unleash` ([Unleash.cs:32](../decompiled/sts2/MegaCrit/sts2/Core/Models/Cards/Unleash.cs#L32)) are both `CardRarity.Basic`. The `RegularEncounter` rarity odds used by SealedDeck only roll Common/Uncommon/Rare; `RollForRarity` ([CardFactory.cs:199-220](../decompiled/sts2/MegaCrit/sts2/Core/Factories/CardFactory.cs#L199-L220)) will never select Basic, so **Necrobinder's identity cards CANNOT appear** in any 30-card SealedDeck pool. Without Bodyguard or Unleash, Necrobinder cannot summon or attack with Osty — the character's entire mechanic is gated by Basic cards that vanilla excludes from the draft. MegaCrit appears not to have thought about this; the streamer-must-include workaround the Discord community asked for is the right fix and the only way to make Necrobinder playable in sealed-deck mode. Patch target: inject Basic-rarity must-includes into `source` ([SealedDeck.cs:31](../decompiled/sts2/MegaCrit/sts2/Core/Models/Modifiers/SealedDeck.cs#L31)) before the grid opens, OR add the must-includes to `player.Deck` directly after the draft completes. Other characters may have similar issues (e.g., Defect's `Zap`/`Dualcast` are Basic too — would need to check each character's StartingDeck for non-Strike-Defend Basics).
...
7. **[RESOLVED 2026-05-12 — answer is bad news] Does MegaCrit filter the Necrobinder pool so the streamer can't end up unplayable?** **No, and they can't, structurally.** Verified via decompile 2026-05-12 after Surfinite asked. The "needed" cards (`Bodyguard`, `Unleash` for Necrobinder; same shape for `Zap`/`Dualcast` on Defect, and likely others) are `CardRarity.Basic`. The SealedDeck pool uses `CardCreationOptions(..., CardRarityOddsType.RegularEncounter)`. `CardFactory.RollForRarity` ([CardFactory.cs:199-220](../decompiled/sts2/MegaCrit/sts2/Core/Factories/CardFactory.cs#L199-L220)) only rolls Common/Uncommon/Rare under RegularEncounter odds — Basic is never selected. So Basic-rarity character-identity cards **cannot appear** in any vanilla SealedDeck draft, regardless of seed. This means streamer re-rolling the seed is not a workaround for the Necrobinder problem; the only fixes are (a) inject Basic must-includes into the pool before the grid opens, (b) add them directly to the deck after the draft. **Implication for the v0.2 sealed-deck-mode spec**: the "configurable per-character must-include list" item from [notes/06](06-followups-and-deferred.md) is not optional polish — it's load-bearing for any character whose StartingDeck contains non-Strike/Defend Basics. Promotes from "nice to have" to "v0.2 sealed-deck-mode must-have".
