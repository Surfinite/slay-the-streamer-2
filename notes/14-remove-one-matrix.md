...
| 6 | Per surface: comply; override a removed card; override a removed Skip; budget 0 unclickable; override during the countdown; chat offline mid-vote (cancel => vanilla screen); the gold "N vote overrides remaining" counter shows during the removal vote AND after the removal on both screens (remove-one/14) | any RemoveOne surface above | `chat removed`; `override: streamer took the removed option`; `override during removal vote`; budget-0 click produces no override log; chat disconnect cancels cleanly to a normal vanilla screen | [ ] |
