...
> 🎮 Tested against Slay the Spire 2 Beta `v0.111.0` and the regular branch `v0.107.1`. One build serves both. On the regular branch the non-combat card-reward rules below are unavailable (the game hook they need isn't there yet), so every card reward is a normal pick vote and the setting says so in the panel.

## 🆕 New in v0.4.2

Custom Mode gets the standard-mode "?" Random character button, which vanilla leaves out. Pick it and the game draws your character from the run seed when you embark, with the same reveal as standard mode (select sound, shake, highlight on the drawn character). Locked until every character is unlocked on your profile, same as vanilla.

## 🆕 New in v0.4.1

Fixes for the regular (non-Beta) branch. The Non-combat card rewards setting is greyed out there, with a note explaining that every card reward is a normal vote until the Beta's game hook reaches the regular branch. The sealed-run Talisman rework now works on the regular branch too (v0.4.0 fell back to the vanilla Talisman there).
...
1. Download `slay_the_streamer_2-v0.4.2.zip` from the assets below, or subscribe on the [Steam Workshop](https://steamcommunity.com/sharedfiles/filedetails/?id=3761888849).
